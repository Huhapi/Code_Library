...
def main():
    ...
    while run:
        ...
        if whiteTurn == False:
            if chessAI.pickApiece(blackTeam,whiteTeam,lastMove) == False:
                run = False
                pygame.quit()
                sys.exit() 
            else:
                piece = chessAI.pickApiece(blackTeam,whiteTeam,lastMove) 
                loc = chessAI.pickAmove(piece, blackTeam, whiteTeam, lastMove)
                
            column = piece.position[1]
            Pieces.Piece.movePiece(piece,loc)
            if piece.name == "P":
